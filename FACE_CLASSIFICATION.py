...
def find_enc(images):
    ...
    for img in images:
        ...
        enc = face_recognition.face_encodings(img)[0]                                           # Function to find the face encodings. Encoding are facial features like eyes, nose, eye-brows, etc.
        enc_list.append(enc)                                           
    return enc_list
...
while True:
    ...
    for encodeframe, facelocframe in zip(face_encodings, face_curr_frame):
        matches = face_recognition.compare_faces(known_end_list, encodeframe)                   # Compares captured encodings with original encodings to find which of the faces does webcam face match.
